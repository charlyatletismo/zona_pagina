...
export const Route = createFileRoute('/trainingTeams/checkTemporary')({
  ...
  loader: async () => {
    const res = await getAuthenticatedThrow<
      z.infer<typeof ARTempTrainingTeamSchema>[]
      >('/api/trainingTeams/temporary',
        z.array(ARTempTrainingTeamSchema));
    const resTrainingTeams = await getAuthenticatedThrow<
    ...
    return { res, resTrainingTeams, resLocations };
  },
...
function RouteComponent() {
  const { res, resTrainingTeams, resLocations } = Route.useLoaderData();
  const navigate = Route.useNavigate();

  const [newTrainingTeam, setNewTrainingTeam] = React.useState(false);
  const [focusedTrainingTeam, setFocusedTrainingTeam] = React.useState<string | null>(null);
  const [focusedUser, setFocusedUser] = React.useState<string | null>(null);

  const columnHelper = createColumnHelper<z.infer<typeof ARTempTrainingTeamSchema>>();
  ...
  const defaultColumns = [
    ...
    columnHelper.display({
      ...
      cell: props => (<div className='flex gap-2'>
        <Button
          onClick={() => {
            setFocusedTrainingTeam(props.row.original.temp);
            setFocusedUser(props.row.original.id);
            setNewTrainingTeam(true);
            setTimeout(() => {
  ...
  return (
    <div className='max-w-full my-2 p-5 mx-auto flex flex-col 2xl:flex-row gap-4'>
      <div>
        {newTrainingTeam && (
          <div className='mb-2 border-b-2'>
            <div className='flex justify-between px-6 pt-6'>
              <div className='text-xl font-bold flex-row md:flex-col'>
                Crear nuevo equipo de entrenamiento
              </div>
              <Button
                onClick={() => {
                  setNewTrainingTeam(false)
                  setFocusedTrainingTeam(null);
                  setFocusedUser(null);
                }}
                variant='outline'
              >
            ...
            <TrainingTeamForm
              dbTrainingTeams={resTrainingTeams.body?.data || []}
              dbLocations={resLocations.body?.data || []}
              trainingTeam={{name: focusedTrainingTeam || ''}}
              onSuccess={async (trainingTeamId) => {
                await postAuthenticated('/api/trainingTeams/updateUser', {
                  userId: focusedUser!,
                  trainingTeamId,
                });
                setNewTrainingTeam(false);
        ...
        <div className='flex flex-col sm:justify-between'>
          <h1 className='text-2xl font-bold mb-4'>Equipos Temporales</h1>
          {res.body.data.length > 0 && (
            <div className='flex gap-2 items-center mb-6 max-w-sm relative'>
      ...
      {res.body.data.length > 0 && (
        <div>
          <div className='flex flex-col sm:justify-between'>
            <h1 className='text-2xl font-bold mb-4'>Equipos existentes</h1>
          </div>
          <TrainingTeamsTable data={resTrainingTeams.body?.data || []} />
        </div>
      )}
    </div>
  )
